...
class Moderation(commands.Cog):
    ...
    async def warn(self, ctx, user: discord.Member, *, reason=None):
        with self.con.cursor() as cursor:
            cursor.execute(
                f"INSERT INTO `WarnsDB` (`gid`, `uid`, `w_reason`, `w_by`, `w_time`) VALUES ('{ctx.guild.id}', '{user.id}', '{reason}', '{ctx.author}', '{time.strftime('%b %d %Y')}')")
        self.con.commit()
        embed = discord.Embed(
    ...
    async def mute(self, ctx, user: discord.Member, duration, *, reason="Причина не указана"):
        with self.con.cursor() as cursor:
            if user == ctx.author:
                ...
                cursor.execute(
                    f"INSERT INTO `MutesDB` VALUES (NULL, '{ctx.guild.id}', '{user.id}', '{reason}', '{timestamp}')")
        self.con.commit()
